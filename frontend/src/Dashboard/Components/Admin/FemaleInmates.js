...
const FemaleInmates = () => {
  ...
  printWindow.document.write(`
    <html>
      <head>
        <title>Female Inmate Details - ${selectedInmate?.inmateCode}</title>
        <style>
          body { 
      ...
      <body>
        <div class="header">
          ...
          <h2>Region 10</h2>
          <h3>FEMALE INMATE DETAILS RECORD - ID: ${selectedInmate?.inmateCode}</h3>
        </div>
    ...
    <Container>
          ...
          <Row className="align-items-center">
            <Col md={12}>
              <InputGroup>
                ...
                <Form.Control
                  type="text"
                  placeholder="Search female inmates..."
                  value={searchQuery}
          ...
          <Row className="mt-2">
            <Col>
              <div className="text-muted small">
                Showing {filteredInmates.length} female inmates
              </div>
            </Col>
          </Row>
        </Card.Body>
      ...
      {isLoading && inmates.length === 0 ? (
        <div className="text-center">
          <Spinner animation="border" role="status">
            <span className="visually-hidden">Loading female inmates...</span>
          </Spinner>
        </div>
      ) : filteredInmates.length === 0 ? (
        <Alert variant="info">
          {searchQuery
            ? 'No female inmates found matching your criteria.' 
            : 'No female inmates found. Add your first female inmate to get started.'}
        </Alert>
      ) : (
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="xl">
        <Modal.Header closeButton>
          <Modal.Title>{editingInmate ? 'Edit Female Inmate' : 'Add New Female Inmate'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Footer>
            ...
            <Button variant="dark" type="submit" disabled={isLoading}>
              {isLoading ? <Spinner size="sm" /> : (editingInmate ? 'Update' : 'Add') + ' Female Inmate'}
            </Button>
          </Modal.Footer>
        </Form>
      </Modal>
      ...
      <Modal show={showViewModal} onHide={() => setShowViewModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Female Inmate Details - {selectedInmate?.inmateCode}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedInmate && (
            <>
