  ...
  const handleCsvUpload = async () => {
    ...
    try {
      ...
      if (response.data.imported > 0) {
        toast.success(`Successfully imported ${response.data.imported} male inmate${response.data.imported !== 1 ? 's' : ''}`);
      } else {
        toast.warning(`No male inmates imported. ${response.data.errors?.length || 0} errors found.`);
      }
